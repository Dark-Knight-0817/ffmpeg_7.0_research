...
int av_codec_is_decoder(const AVCodec *avcodec) // 检查给定的 AVCodec 是否为解码器
{
    const FFCodec *const codec = ffcodec(avcodec);
...
const char *avcodec_get_name(enum AVCodecID id) // 目的是根据给定的编解码器ID（AVCodecID）返回对应的编解码器名称
{
...
/**
* 获取编解码器支持的硬件配置信息
* 通过递增 index 可以遍历所有支持的硬件配置
*
* @param avcodec 编解码器
* @param index   硬件配置的索引,从0开始
* @return 成功返回对应的硬件配置,如果 index 超出范围或没有硬件配置则返回 NULL
*/
const AVCodecHWConfig *avcodec_get_hw_config(const AVCodec *avcodec, int index)
{
    ...
    // 如果编解码器没有硬件配置或索引小于0,返回NULL
    if (!codec->hw_configs || index < 0)
        return NULL;

    // 遍历硬件配置数组,直到指定索引
    // 如果在到达指定索引之前发现NULL配置,说明索引超出了范围
    for (i = 0; i <= index; i++)
