...
typedef struct AVCodecParser {
    /* 允许的编解码器ID数组
     * 一个解析器可能支持多个相关的编解码器 */
    int codec_ids[7]; /* several codec IDs are permitted. 允许多个编解码器ID */
    /**
     * 私有数据的大小
     * 用于为解析器分配额外的内存空间
     */
    ...
    /**
     * 解析函数
     * 这个回调从不返回错误，负值表示帧开始于前一个数据包
     * @param s 解析器上下文
     * @param avctx 编解码器上下文
     * @param poutbuf 输出缓冲区的指针
     * @param poutbuf_size 输出缓冲区大小的指针
     * @param buf 输入缓冲区
     * @param buf_size 输入缓冲区大小
     * @return 解析结果
     */
