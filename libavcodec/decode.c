...
int ff_decode_receive_frame_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    // 确保输出的 AVFrame 没有任何缓冲区
    av_assert0(!frame->buf[0]);

    // 如果编解码器支持 receive_frame 回调函数
    if (codec->cb_type == FF_CODEC_CB_TYPE_RECEIVE_FRAME) {
        ...
        // 如果编解码器不支持 receive_frame 回调函数，使用默认的解码接收函数
        ret = decode_simple_receive_frame(avctx, frame);

    // 如果返回 AVERROR_EOF，表示解码已经结束
    if (ret == AVERROR_EOF)
        avci->draining_done = 1;
...
/**
 * @brief FFmpeg 解码过程中的关键函数，用于向解码器发送一个压缩的数据包
 */
int attribute_align_arg avcodec_send_packet(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    /*  检查编解码器上下文是否已打开，且是否为解码器    */
    if (!avcodec_is_open(avctx) || !av_codec_is_decoder(avctx->codec))
        return AVERROR(EINVAL);

    /*  如果已经开始排空（draining），返回 EOF 错误 */
    ...
    if (avpkt && (avpkt->data || avpkt->side_data_elems)) {
        /*  如果内部缓冲包非空，返回 EAGAIN 错误    */
        if (!AVPACKET_IS_EMPTY(avci->buffer_pkt))
            return AVERROR(EAGAIN);
        /*  将输入包复制到内部缓冲包    */
        ret = av_packet_ref(avci->buffer_pkt, avpkt);   // avci->buffer_pkt 并不是一个队列，它是解码器内部的一个单包缓冲
        if (ret < 0)
            return ret;
    } else
        /*  如果没有输入包，标记开始排空过程    */
