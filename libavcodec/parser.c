...
/**
 *  初始化一个 AVCodecParserContext. 主要目的是为特定的编解码器ID创建和初始化一个解析器上下文。
 *  这个解析器上下文后续会用于从原始编码数据中解析出单独的编码单元（如视频帧或音频包）
 */
AVCodecParserContext *av_parser_init(int codec_id) 
{
    ...
    while ((parser = av_parser_iterate(&i))) { // 使用自定义简单的迭代器
...
found:
    s = av_mallocz(sizeof(AVCodecParserContext)); //  FFmpeg 中进行底层数据解析的核心结构之一，它提供了必要的上下文信息和状态跟踪，使得从原始编码数据流中提取有意义的编码单元成为可能
    if (!s)
        goto err_out;
    s->parser = parser;
