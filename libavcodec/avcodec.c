...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **options) // 核心函数，负责初始化和打开编解码器
{
    int ret = 0;
...
/**
 * @brief FFmpeg 中用于接收解码后的帧或编码前的帧的通用接口
 */
int attribute_align_arg avcodec_receive_frame(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    /*  检查编解码器类型并调用相应的函数    */
    if (av_codec_is_decoder(avctx->codec))
        return ff_decode_receive_frame(avctx, frame); // 如果是解码器，调用解码接收帧函数
    return ff_encode_receive_frame(avctx, frame); // 如果不是解码器（即是编码器），调用编码接收帧函数
}
