...
static void get_frame_defaults(AVFrame *frame)
{
    ...
    frame->pts                   =                  // 设置帧的时间戳字段为 AV_NOPTS_VALUE，表示当前帧没有有效的显示时间戳。
    frame->pkt_dts               = AV_NOPTS_VALUE;  // 设置帧的解码时间戳（dts）为 AV_NOPTS_VALUE，表示没有有效的解码时间戳。
    frame->best_effort_timestamp = AV_NOPTS_VALUE;  // 最佳的时间戳值，通常用于估计时序不确定的帧，初始化为 AV_NOPTS_VALUE。
    frame->duration            = 0;                 // 帧的持续时间初始化为 0。
#if FF_API_FRAME_PKT
FF_DISABLE_DEPRECATION_WARNINGS
    frame->pkt_pos             = -1;                // 设置帧数据包的偏移位置，旧 API 下默认值为 -1，表示位置未知或无效。
    frame->pkt_size            = -1;                // 设置帧数据包的大小，旧 API 下默认值为 -1，表示大小未知。
FF_ENABLE_DEPRECATION_WARNINGS
#endif
    frame->time_base           = (AVRational){ 0, 1 }; // 设置时间基为无效值 {0, 1}，表示时间基还未设置。
    frame->sample_aspect_ratio = (AVRational){ 0, 1 }; // 设置样本纵横比为无效值 {0, 1}，表示纵横比还未设置。
    frame->format              = -1; /* unknown */     // 设置帧的格式为未知，-1 表示帧格式尚未指定（例如，像素格式或样本格式）。
    frame->extended_data       = frame->data;          // 初始化扩展数据指针为基本数据指针，通常用于多声道音频或非平面视频。
    frame->color_primaries     = AVCOL_PRI_UNSPECIFIED;// 设置颜色原色特性为未指定。
    frame->color_trc           = AVCOL_TRC_UNSPECIFIED;// 设置颜色传递特性为未指定。
    frame->colorspace          = AVCOL_SPC_UNSPECIFIED;// 设置色彩空间为未指定。
    frame->color_range         = AVCOL_RANGE_UNSPECIFIED;// 设置颜色范围为未指定（例如，可能是有限的或全范围的颜色）。
    frame->chroma_location     = AVCHROMA_LOC_UNSPECIFIED;// 设置色度样本位置为未指定。
    frame->flags               = 0;                    // 初始化帧的标志位为 0，表示当前帧没有任何特殊的标志。
}
...
{
    AVFrame *frame = av_malloc(sizeof(*frame)); // 创建 frame

    if (!frame)
        return NULL;

    get_frame_defaults(frame); // 设置 frame 参数,对其进行初始化
