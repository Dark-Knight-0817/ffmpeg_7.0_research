...
/**
 * Copy data from an hwframe context to an AVFrame.
 * HW -> SW
 * @param dst 目标帧
 * @param src 源帧
 * @param flags 传输标志位
...
int av_hwframe_transfer_data(AVFrame *dst, const AVFrame *src, int flags)   
{
    ...
    // 如果目标帧还没有分配缓冲区，调用 transfer_data_alloc 进行分配
    if (!dst->buf[0])
