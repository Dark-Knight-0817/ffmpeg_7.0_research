...
static int size_mult(size_t a, size_t b, size_t *r) // 安全的乘法函数，用于检测大小计算中的整数溢出
{
    size_t t;

#if (!defined(__INTEL_COMPILER) && AV_GCC_VERSION_AT_LEAST(5,1)) || AV_HAS_BUILTIN(__builtin_mul_overflow)
    if (__builtin_mul_overflow(a, b, &t)) // 使用编译器提供的内建函数来检测乘法溢出. 如果发生溢出，返回错误
...
void *av_realloc(void *ptr, size_t size) // 重新分配ptr指向的内存,增加or减少
{
    void *ret;
    if (size > atomic_load_explicit(&max_alloc_size, memory_order_relaxed)) // 内存大小检查
        return NULL;

#if HAVE_ALIGNED_MALLOC
    ret = _aligned_realloc(ptr, size + !size, ALIGN);
#else
    ret = realloc(ptr, size + !size);   // 重新分配内存
#endif
#if CONFIG_MEMORY_POISONING // 内存中毒检测
    if (ret && !ptr)
...
int av_reallocp(void *ptr, size_t size)
{
    ...
    if (!size) {
        av_freep(ptr); // 释放内存,置为 NULL
        return 0;
    }

    memcpy(&val, ptr, sizeof(val));
    val = av_realloc(val, size); // 重新分配内存
...
void *av_realloc_array(void *ptr, size_t nmemb, size_t size) // 检测是否溢出,正常则分配内存
{
    size_t result;
    if (size_mult(nmemb, size, &result) < 0) // 检测乘法是否发出溢出
        return NULL;
    return av_realloc(ptr, result); // 分配内存
}
...
void av_freep(void *arg) // 内存管理函数，用于释放指针指向的内存并将指针本身设置为 NULL
{
    void *val;
...
char *av_strdup(const char *s) // 字符串更新
{
    char *ptr = NULL;
    if (s) {
        size_t len = strlen(s) + 1;
        ptr = av_realloc(NULL, len); // 重新分配内存,增加or减少
        if (ptr)
            memcpy(ptr, s, len);
    }
    return ptr;
