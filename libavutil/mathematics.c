...
int64_t av_rescale_rnd(int64_t a, int64_t b, int64_t c, enum AVRounding rnd) // 用于精确的大数缩放计算的函数，主要用于时间戳和采样率的转换
{
    int64_t r = 0;
    av_assert2(c > 0);
    av_assert2(b >=0);
...
{
    return av_rescale_rnd(a, b, c, AV_ROUND_NEAR_INF); // 用于精确的大数缩放计算的函数，主要用于时间戳和采样率的转换
}
