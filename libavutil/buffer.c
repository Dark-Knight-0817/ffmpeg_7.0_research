...
/**
 * 创建一个新的 AVBufferRef 引用现有的 AVBuffer
 * 此函数增加底层 AVBuffer 的引用计数
 *
 * @param buf 要引用的 AVBufferRef
 * @return 成功时返回新的 AVBufferRef,失败时返回 NULL
 */
AVBufferRef *av_buffer_ref(const AVBufferRef *buf)
{
    // 分配新的 AVBufferRef 结构体空间,并初始化为0
    ...
    // 将输入 buf 的内容复制到新的 AVBufferRef
    // 这会复制 buffer 指针和其他相关数据
    *ret = *buf;

    // 原子操作增加底层 AVBuffer 的引用计数
    // memory_order_relaxed 表示这是一个宽松的内存序
    // 因为引用计数的增减本身就是原子的,不需要额外的内存屏障
