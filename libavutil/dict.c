...
const AVDictionaryEntry *av_dict_iterate(const AVDictionary *m,
                                         const AVDictionaryEntry *prev) //  FFmpeg 库中用于遍历 AVDictionary 的函数
{
    int i = 0;
...
AVDictionaryEntry *av_dict_get(const AVDictionary *m, const char *key,
                               const AVDictionaryEntry *prev, int flags) // 在 AVDictionary（FFmpeg 的字典数据结构）中匹配特定的键（key）
{
    ...
    while ((entry = av_dict_iterate(m, entry))) { // 遍历字典中的所有条目
        const char *s = entry->key;
        if (flags & AV_DICT_MATCH_CASE) // 根据标志选择匹配方式
            for (j = 0; s[j] == key[j] && key[j]; j++) // 区分大小写的匹配
                ;
        else
            for (j = 0; av_toupper(s[j]) == av_toupper(key[j]) && key[j]; j++) // 不区分大小写的匹配
                ;
        if (key[j]) // 如果 key 还没有结束，说明不匹配，继续下一个条目
            continue;
        if (s[j] && !(flags & AV_DICT_IGNORE_SUFFIX)) // 如果条目的键比搜索的键长，且不忽略后缀，则继续下一个条目
            continue;
        return (AVDictionaryEntry *)entry; // 找到匹配的条目，返回它
    }
    return NULL; // 没有找到匹配的条目，返回 NULL
}

int av_dict_set(AVDictionary **pm, const char *key, const char *value,
                int flags) //  FFmpeg 库中用于设置或修改 AVDictionary 中的键值对的函数
{
    AVDictionary *m = *pm;
...
int av_dict_copy(AVDictionary **dst, const AVDictionary *src, int flags) // 复制 AVDictionary(一种键值对)
{
    const AVDictionaryEntry *t = NULL;

    while ((t = av_dict_iterate(src, t))) { // 
        int ret = av_dict_set(dst, t->key, t->value, flags); // 用于设置或修改 AVDictionary 中的键值对的函数
        if (ret < 0)
            return ret;
    }
