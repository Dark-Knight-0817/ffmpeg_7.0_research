...
void init_dynload(void) // 初始化动态加载相关功能的函数
{
#if HAVE_SETDLLDIRECTORY && defined(_WIN32) // 确保仅在支持 SetDllDirectory 函数的 Windows 系统中编译
    /* Calling SetDllDirectory with the empty string (but not NULL) removes the
     * current working directory from the DLL search path as a security pre-caution. */
    SetDllDirectory("");    // 这个调用会清空当前工作目录（CWD）在 DLL 搜索路径中的位置。这是一种安全措施，以防止恶意软件在当前工作目录中放置与标准库同名的 DLL 文件，从而被错误地加载。
#endif
}
...
int parse_options(void *optctx, int argc, char **argv, const OptionDef *options,
                  int (*parse_arg_function)(void *, const char*)) // 分析所有可选项
{
    const char *opt;
    int optindex, handleoptions = 1, ret;

    /* perform system-dependent conversions for arguments list */
    prepare_app_arguments(&argc, &argv);    // 系统相关参数转换
    ...
    while (optindex < argc) {
        ...
        if (handleoptions && opt[0] == '-' && opt[1] != '\0') {
            ...
            if ((ret = parse_option(optctx, opt, argv[optindex], options)) < 0) // 分析单个可选项
                return ret;
            optindex += ret;
        } else {
...
int locate_option(int argc, char **argv, const OptionDef *options,
                  const char *optname)  // 在命令行中定位特定选项
{
    const OptionDef *po;
    int i;
...
static void check_options(const OptionDef *po) // 检查命令行选项（参数可选项）的有效性
{
    while (po->name) {
...
void parse_loglevel(int argc, char **argv, const OptionDef *options)
{
    ...
    check_options(options); // 检查命令行选项（参数可选项）的有效性
...
int stream_specifier_parse(StreamSpecifier *ss, const char *spec,
                           int allow_remainder, void *logctx) // 解析流指定符（stream specifier). 流指定符是一种用于精确选择特定音频、视频或其他类型流的字符串表示法
{
    char *endptr;
    int ret;

    /*  初始化 StreamSpecifier 结构  */
    memset(ss, 0, sizeof(*ss));

    ss->idx         = -1;                    // 初始化流索引为 -1（未指定）
    ss->media_type  = AVMEDIA_TYPE_UNKNOWN;  // 初始化媒体类型为未知
    ss->stream_list = STREAM_LIST_ALL;       // 初始化为所有流列表

    av_log(logctx, AV_LOG_TRACE, "Parsing stream specifier: %s\n", spec);

    while (*spec) {
        if (*spec <= '9' && *spec >= '0') { /* opt:index  解析流索引  */
            ...
            break; // 索引解析后终止
                   ...
                   !cmdutils_isalnum(*(spec + 1))) { /* opt:[vasdtV]  解析媒体类型 */
        ...
        } else if (!strncmp(spec, "disp:", 5)) {  // 解析处置指定符
            ...
            // this terminates the specifier
            break;
        } else if (*spec == 'm' && *(spec + 1) == ':') { // 解析元数据指定符
                                ...
                                void *logctx) // 检查给定的流是否匹配指定的流选择器
{
    ...
    case STREAM_LIST_STREAM_ID:
        // <n-th> stream with given ID makes no sense and should be impossible to request. 确保索引未指定
        av_assert0(ss->idx < 0);
        // return early if we know for sure the stream does not match. 如果流ID不匹配，直接返回0
        if (st->id != ss->list_id)
            return 0;
        start_stream = st->index;
        nb_streams   = st->index + 1;
        break;
    case STREAM_LIST_ALL: // 如果指定了索引，从头开始搜索；否则只检查当前流
        start_stream = ss->idx >= 0 ? 0 : st->index;
        nb_streams   = st->index + 1;
        break;
    case STREAM_LIST_PROGRAM:  // 查找指定ID的程序
        ...
        break;
    case STREAM_LIST_GROUP_ID: // 查找指定ID的流组
        ...
        // fall-through
    case STREAM_LIST_GROUP_IDX: // 如果是按索引查找流组
...
int check_stream_specifier(AVFormatContext *s, AVStream *st, const char *spec)
{
    ...
    ret = stream_specifier_parse(&ss, spec, 0, NULL); // 解析流指定符（stream specifier).
    if (ret < 0)
        return ret;

    ret = stream_specifier_match(&ss, s, st, NULL); // 检查给定的流是否匹配指定的 流选择器
    stream_specifier_uninit(&ss);
    return ret;
}
...
int filter_codec_opts(const AVDictionary *opts, enum AVCodecID codec_id,
                      ...
                      AVDictionary **dst, AVDictionary **opts_used) // 设置一些初始变量和确定编解码器选项的类型
{
    ...
    char          prefix = 0;
    const AVClass    *cc = avcodec_get_class(); // 返回 AVCodecContext 的 AVClass
...
int setup_find_stream_info_opts(AVFormatContext *s,
                                ...
                                AVDictionary ***dst) // 为每个流创建一个经过过滤的编解码器选项字典
{
    int ret;
    AVDictionary **opts;
    ...
    *dst = NULL;
    ...
    for (int i = 0; i < s->nb_streams; i++) {
        ret = filter_codec_opts(local_codec_opts, s->streams[i]->codecpar->codec_id,
                                s, s->streams[i], NULL, &opts[i], NULL); // 设置一些初始变量和确定编解码器选项的类型
        if (ret < 0)
            goto fail;
    }
    *dst = opts;
...
void remove_avoptions(AVDictionary **a, AVDictionary *b) // 从一个字典中移除另一个字典中存在的所有键
{
    const AVDictionaryEntry *t = NULL;
...
int check_avoptions(AVDictionary *m) // 检查一个字典是否为空，如果不为空，则报告第一个未处理的选项
{
