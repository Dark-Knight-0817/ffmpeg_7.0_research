...
int attribute_align_arg swr_convert(struct SwrContext *s,
                                          uint8_t * const *out_arg, int out_count,      // 输出缓冲区及采样数 
                                    const uint8_t * const *in_arg,  int in_count)       // 输入缓冲区及采样数
{
    AudioData * in= &s->in;     // 输入音频数据结构
    AudioData *out= &s->out;    // 输出音频数据结构 
...
#define MAX_DROP_STEP 16384         // 每次最多丢弃16384个样本
        if((ret=swri_realloc_audio(&s->drop_temp, FFMIN(s->drop_output, MAX_DROP_STEP)))<0) // 为临时缓冲区分配内存
            return ret;

        reversefill_audiodata(&s->drop_temp, tmp_arg);  // 反向填充临时缓冲区
        s->drop_output *= -1; //FIXME find a less hackish solution  // 标记处理中
        // 转换并丢弃样本
    ...
    if(!in_arg){
        ...
        fill_audiodata(in ,  (void*)in_arg);    // 填充输入音频数据

    fill_audiodata(out, out_arg);   // 填充输出音频数据
    ...
    if(s->resample){
        ...
        if(ret>0 && !s->drop_output)
            s->outpts += ret * (int64_t)s->in_sample_rate;  // 更新输出PTS
...
/**
* 设置重采样补偿("软"补偿)，用于变速处理
* 内部由 swr_next_pts() 按需调用
* 
* @param s 已分配的 SwrContext。如未初始化或未设置 SWR_FLAG_RESAMPLE 标志,
*          会调用 swr_init() 并设置该标志
* @param sample_delta 每个样本的 PTS 增量
*                    >0 表示加速
*                    <0 表示减速
*                    表示相邻样本间 PTS 的差值变化量
* @param compensation_distance 需要补偿的样本数量
*                            在这些样本内完成速度调整
*                            值越小变速越快,但可能产生音频失真
*                            值越大变速越平滑
* @return >=0 成功
...
*         - compensation_distance 为 0 但 sample_delta 不为 0
*         - 重采样器不支持补偿
*         - swr_init() 调用失败
*/
