...
typedef struct AVProbeData {
    const char *filename; // 要探测的文件名
    unsigned char *buf; /**< Buffer must have AVPROBE_PADDING_SIZE of extra allocated bytes filled with zero. 指向包含文件开头数据的缓冲区,用于分析文件的魔数（magic numbers）或特征序列 */
    int buf_size;       /**< Size of buf except extra allocated bytes. 表示 buf 中实际数据的大小（不包括额外分配的填充字节） */
    const char *mime_type; /**< mime_type, when known. 可以帮助更准确地识别文件格式 */
} AVProbeData; // 用于存储和传递探测文件格式所需的数据
