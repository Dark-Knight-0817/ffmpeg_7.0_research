...
/**
 * @brief FFmpeg 中用于建立 URL 连接的核心函数，它会根据已经建立好的 URLContext 结构体来实际建立连接 
 */
int ffurl_connect(URLContext *uc, AVDictionary **options)
{
    ...
    /**
     * @brief 实际调用协议的 url_open 函数建立连接
     * @param url_open 较早的接口，只接受3个参数：URLContext、文件名、标志
     * @param url_open2 新的接口，接受4个参数：URLContext、文件名、标志、选项
     * @note 这是一种向后兼容的设计，允许旧协议处理器使用较旧的接口
     */
    err =
...
/**
 * @brief FFmpeg中根据 URL 确定使用哪个协议处理器的关键函数
 * @note 函数接收一个URL字符串输入，解析它以确定应该使用哪个协议处理器，然后返回对应的 URLProtol 结构体指针
 */
static const struct URLProtocol *url_find_protocol(const char *filename)
{
    ...
    /**
     * @brief 计算URL开头部分中属于有效协议字符的长度，
     * @param URL_SCHEME_CHARS 通常定义为字母、数字、加号、点、减号
     */
     ...
     *  @brief 确定协议字符串
     *  确定自协议字符串，
     *  如果URL没有遵循 protocol:path 格式（协议后没冒号）
     *  或者它是windows风格的文件路径，如："C:\path\to\file",
     *  或者不是以"subfile"，开头且包含冒号的特殊格式，
     *  则默认使用 "file"协议，否则，将URL开头直到冒号的部分作为协议名称
     */
    if (filename[proto_len] != ':' &&
        ...
        /**
         * @brief 处理嵌套协议的情况，如,"http+rtsp://",
         *        这种情况下，“http+rtsp”是完整的协议字符串，而“http”是嵌套协议。
         *        代码将 proto_nested 设置为加号前的部分
         */
    ...
    if (!protocols)
        return NULL;
    /**
     * @brief 遍历协议列表，查找与URL匹配的协议处理器
     * @note 两种情况：
     *        1. 精确匹配完整协议名(如"http+rtsp"")
     *        2. 如果协议支持嵌套方案，则尝试匹配嵌套协议名（如"http"）
     */
...
/**
 * @brief Open a new URL context for accessing a resource indicated by
 *       url. Any previous context opened by the same avio_open() instance
 *       will be closed automatically.
 *      FFmpeg中处理URL打开操作的核心函数，负责创建和建立网络连接。
 * @param puc 指向URLContext结构体的指针，用于存储打开的URL上下文
 * @param filename 要打开的URL
 * @param flags 打开URL的标志，如读写权限等
 * @param int_cb 中断回调函数,用于处理长时间操作的中断
 * @param options 打开URL的选项，包含各种配置参数
 * @param whitelist 白名单，用于指定允许使用的协议白名单
 * @param blacklist 黑名单，用于指定禁止使用的协议黑名单
 * @param parent 父URL上下文，用于嵌套协议
 */
                         ...
                         URLContext *parent)
{
    ...
    AVDictionaryEntry *e;
    /* 分配URLContext */
    int ret = ffurl_alloc(puc, filename, flags, int_cb);// 解析URL字符串来确定使用哪个协议
    if (ret < 0)
        return ret;
    if (parent) {
        /* 继承父的上下文属性 */
                                         ...
                                         int read)
{
    ...
    len = 0;
    while (len < size_min) {
        if (ff_check_interrupt(&h->interrupt_callback)) // 返回1 退出函数里面的循环，返回0 继续内部任务
            return AVERROR_EXIT;
