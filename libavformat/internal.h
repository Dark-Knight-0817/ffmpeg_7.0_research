...
typedef struct FFFormatContext {
    /**
     * The public context.
     * 这是一个AVFormatContext结构，包含了格式相关的公共信息
     */
    AVFormatContext pub;
     ...
     */
    enum {
        AVOID_NEGATIVE_TS_DISABLED = -1, // 禁用避免负时间戳
        AVOID_NEGATIVE_TS_UNKNOWN  = 0,  // 避免负时间戳状态未知
        AVOID_NEGATIVE_TS_KNOWN    = 1,  // 避免负时间戳状态已知
...
#define AVOID_NEGATIVE_TS_ENABLED(status) ((status) >= 0)
     ...
     * av_seek_frame() 支持 */
    int64_t data_offset; /**< offset of the first packet. 第一个数据包的偏移量  */
     ...
     * 
     * 通用代码使用这个作为临时数据包
     * 用于解析数据包或进行复用，特别是在刷新时。
     * 对于解复用器，它也可能在短时间内用于其他目的，
     * 这些用途保证不会与 av_read_frame() (或 ff_read_packet()) 的调用重叠，
     * 也不会相互重叠。
     * 解复用器可以使用它来替代栈上的数据包
     * (除非它们使用自己的AVFormatContext调用上述函数之一)。
     * 每个使用者都必须确保在使用后这个数据包是空白的。
     */
    AVPacket *parse_pkt;
...
{
    return (FFFormatContext*)s; // 返回一个扩展的格式上下文结构
}
