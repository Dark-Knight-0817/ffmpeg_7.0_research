...
{
    RTSPState *rt = s->priv_data;               // 获取RTSP状态信息
    RTSPMessageHeader reply1, *reply = &reply1; // 定义响应消息头
    int i;
    char cmd[MAX_URL_SIZE];                     // 用于存储RTSP命令

    av_log(s, AV_LOG_DEBUG, "hello state=%d\n", rt->state);
    rt->nb_byes = 0;                            // 重置BYE计数器
        ...
        if (rt->transport == RTSP_TRANSPORT_RTP) {
            ...
            for (i = 0; i < rt->nb_rtsp_streams; i++) {
                ...
                if (!rtpctx)
                    continue;
                /*  重置RTP相关参数 */
                ff_rtp_reset_packet_queue(rtpctx);  // 重置包队列
                rtpctx->last_rtcp_ntp_time  = AV_NOPTS_VALUE;   // 重置RTCP时间戳
                rtpctx->first_rtcp_ntp_time = AV_NOPTS_VALUE;
                rtpctx->base_timestamp      = 0;    // 重置基础时间戳
                rtpctx->timestamp           = 0;    // 重置当前时间戳
                rtpctx->unwrapped_timestamp = 0;    // 重置展开后的时间戳
                rtpctx->rtcp_ts_offset      = 0;    // 重置RTCP时间戳偏移
            }
        }
        /*  构造PLAY命令    */
...
{
    RTSPState *rt = s->priv_data;   // 获取RTSP状态信息
    RTSPMessageHeader reply1, *reply = &reply1; // 定义响应消息头部
