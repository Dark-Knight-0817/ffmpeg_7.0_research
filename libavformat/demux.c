...
static const AVCodec *find_probe_decoder(AVFormatContext *s, const AVStream *st, enum AVCodecID codec_id) // 为给定的编解码器ID找到一个合适的解码器，特别是在需要进行探测（probing）的情况下
{
    const AVCodec *codec;
...
static int init_input(AVFormatContext *s, const char *filename,
                      AVDictionary **options) // 用来初始化输入并确定输入文件的格式类型的
{
    int ret;
    ...
    if (s->pb) {
        ...
        /*  av_probe_input_buffer2 从现有的 I/O 上下文读取数据并探测格式 */
        if (!s->iformat)
            return av_probe_input_buffer2(s->pb, &s->iformat, filename,
                                          s, 0, s->format_probesize); // 对输入的文件探测文件类型,并为其分配缓冲区
    ...
    /*  在没有 I/O 上下文的情况下进行的,只使用文件名（可能还有一些初始数据）来猜测格式,
        适用于不需要实际打开文件就可以确定格式的情况（如基于文件扩展名）*/
    if ((s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format2(&pd, 0, &score)))) // 通过综合分析文件名、内容和已知格式的特征来确定输入数据的最可能格式
        return score;
...
int avformat_open_input(AVFormatContext **ps, const char *filename,
                        const AVInputFormat *fmt, AVDictionary **options) // 打开媒体文件，读取文件头信息，初始化解复用器
{
    FormatContextInternal *fci;
    AVFormatContext *s = *ps;
    FFFormatContext *si;
    AVDictionary *tmp = NULL; // 指向键值对,临时字典，用于存储选项
    ID3v2ExtraMeta *id3v2_extra_meta = NULL;
    int ret = 0;

    /*  如果没有提供AVFormatContext，则分配一个新的 */
    if (!s && !(s = avformat_alloc_context())) // 当s为空，则分配一个 avformatcontext 上下文
        return AVERROR(ENOMEM);
    /*  获取内部格式上下文  */
    fci = ff_fc_internal(s); // 用于从 AVFormatContext 获取 FormatContextInternal 结构
    si = &fci->fc;

    /*  检查AVFormatContext是否正确分配 */
    if (!s->av_class) {
    ...
    /*  复制选项字典  */
    if (options)
        av_dict_copy(&tmp, *options, 0); // 复制 AVDictionary 的函数

    /*  如果已有I/O上下文，设置自定义I/O标志  */
    ...
    /* 复制文件名 */
    if (!(s->url = av_strdup(filename ? filename : ""))) { // 更新字符串
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    */--
    if (s->iformat->flags & AVFMT_NEEDNUMBER) {
        if (!av_filename_number_test(filename)) { // 测试文件名是否为可用的
            ret = AVERROR(EINVAL);
            goto fail;
        }
    }
...
static int has_codec_parameters(const AVStream *st, const char **errmsg_ptr) // 检查给定的 AVStream 是否包含足够的编解码器参数信息。
{
    const FFStream *const sti = cffstream(st);
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options) // 分析输入媒体文件（或流）的内容，以获取有关其中包含的音视频流的详细信息
{
    FFFormatContext *const si = ffformatcontext(ic);  // 返回一个扩展的格式上下文结构
    ...
    for (unsigned i = 0; i < ic->nb_streams; i++) {
        ...
        /*
         * 检查是否需要为流初始化解析器
         * 如果需要，则初始化解析器并根据流的需求配置它
         * 如果无法初始化所需的解析器，则发出警告   */
        if (!sti->parser && !(ic->flags & AVFMT_FLAG_NOPARSE) && sti->request_probe <= 0) { // 当前流还没有解析器 + 格式上下文没有设置 "不解析" 标志 + 流不需要额外的探测
            sti->parser = av_parser_init(st->codecpar->codec_id); // 为特定的编解码器ID创建和初始化一个解析器上下文
            if (sti->parser) {  // 
                if (sti->need_parsing == AVSTREAM_PARSE_HEADERS) { // 如果流需要解析头部
                    sti->parser->flags |= PARSER_FLAG_COMPLETE_FRAMES;
                } else if (sti->need_parsing == AVSTREAM_PARSE_FULL_RAW) { // 如果流需要完全原始解析
                    sti->parser->flags |= PARSER_FLAG_USE_CODEC_TS;
                }
            } else if (sti->need_parsing) {
                av_log(ic, AV_LOG_VERBOSE, "parser not found for codec "
                       "%s, packets or times may be invalid.\n",
                       avcodec_get_name(st->codecpar->codec_id)); // 目的是根据给定的编解码器ID（AVCodecID）返回对应的编解码器名称
            }
        }

        ret = avcodec_parameters_to_context(avctx, st->codecpar); // 将 AVCodecParameters 结构中的参数复制到 AVCodecContext 结构中
        if (ret < 0)
            goto find_stream_info_err;
        if (sti->request_probe <= 0)
            sti->avctx_inited = 1;

        codec = find_probe_decoder(ic, st, st->codecpar->codec_id); // 为给定的编解码器ID找到一个合适的解码器，特别是在需要进行探测（probing）的情况下

        /* Force thread count to 1 since the H.264 decoder will not extract
         * SPS and PPS to extradata during multi-threaded decoding.
         *
         * 这是为了解决H.264解码器在多线程解码时无法提取SPS（Sequence Parameter Set）和PPS（Picture Parameter Set）到extradata的问题
         * 通过将线程数强制设置为1，确保可以正确提取这些重要的参数  */
        ...
        /**
         * 这部分代码尝试打开解码器，主要在两种情况下：
         *  a. 当流没有完整的编解码器参数，并且不需要进一步探测时。
         *  b. 对于字幕流。
         *  使用avcodec_open2函数尝试打开解码器。
         *  如果打开失败，记录一个警告日志
         */
        if (!has_codec_parameters(st, NULL) && sti->request_probe <= 0 ||
            st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
            if (codec && !avctx->codec)
                if (avcodec_open2(avctx, codec, options ? &options[i] : &thread_opt) < 0) // 核心函数，负责初始化和打开编解码器
                    av_log(ic, AV_LOG_WARNING,
    ...
    for (;;) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
             ...
             * 这段代码主要用于：
             * 1. 确保正确检测到帧延迟和B帧。
             * 2. 确保提取了必要的额外数据（如编解码器配置）。
             * 3. 获取足够的时间戳信息，特别是对于视频和音频流。
             * 4. 在某些情况下限制分析的帧数，以平衡分析的深度和效率。
             *
             * @param sti 流信息结构体指针
             * @param st 流结构体指针
             * @param ic 输入格式上下文指针
             * @param count 当前已分析的帧数
             * @return 如果需要继续分析则返回true，否则返回false
             */
        ...
        /*  如果所有流都已分析完毕，且没有缺失的流，设置analyzed_all_streams标志    */
        analyzed_all_streams = 0;
        if (i == ic->nb_streams && !si->missing_streams) {
            analyzed_all_streams = 1;
            /* NOTE: If the format has no header, then we need to read some
             * packets to get most of the streams, so we cannot stop here.
                如果格式没有头部，我们可能需要继续读取一些数据包    */
         ...
         * (AVFMTCTX_NOHEADER). 
            读取下一个数据包*/
        ret = read_frame_internal(ic, pkt1);    // 读取音视频的数据包
        if (ret == AVERROR(EAGAIN))
            continue;
