...
int av_filename_number_test(const char *filename) // 测试文件名是否包含有效的帧号格式的函数
{
    char buf[1024];
    return filename &&
           (av_get_frame_filename(buf, sizeof(buf), filename, 1) >= 0); // 拿到文件名
}
...
int ff_get_frame_filename(char *buf, int buf_size, const char *path, int64_t number, int flags) // 用于生成帧文件名的工具函数
{
    const char *p;
    char *q, buf1[20], c;
...
{
    return ff_get_frame_filename(buf, buf_size, path, number, 0); // 用于生成帧文件名的工具函数
}
...
#if CONFIG_NETWORK
    int ret;
    if ((ret = ff_network_init()) < 0)  // 初始化网络
        return ret;
    if ((ret = ff_tls_init()) < 0)  // 初始化网络通信安全
        return ret;
#endif
    return 0;
}
