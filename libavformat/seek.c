...
/**
 *  @note  ts seek 逻辑是：
 * 给定一个文件位置，直接将文件指针指向该位置。
 * 接下来调用 read_packet()读取一个 ts 包(188byte)时,由于之前进行了 seek 操作，
 * 文件指针很可能没有指到一个 ts packet 的包头位置(包头以0x47 byte打头的)，
 * 这时候需要调用 mpegts_resync() 进行重新同步找到包头，然后再重新读取一个完整 ts packet.
*/
// ts相关调用：avformat_seek_file()->av_seek_frame()->seek_frame_internal()->seek_frame_byte()
...
/**
* @brief 高级的seek操作函数，支持指定时间范围的seek
* @param s: 上下文
* @param stream_index: 要seek的流索引,如果是-1表示选择默认流
* @param min_ts: 最小时间戳
* @param ts: 目标时间戳  
* @param max_ts: 最大时间戳
* @param flags: seek标志位
* @return 成功返回>=0,失败返回错误码
* @li read_seek2 (新接口):支持范围seek、可以指定min_ts和max_ts、精度更高
* @li read_timestamp:通过二分查找实现seek、可以精确定位时间戳
* @li read_seek (旧接口):基础的seek功能、只能指定目标时间戳
*/
/**
 ...
 * 
 * @note This is part of the new seek API which is still under const ruction.
                       ...
                       int64_t ts, int64_t max_ts, int flags)
{
    ...
    if (ffifmt(s->iformat)->read_seek2) {
        ...
        /*  如果是默认流且只有一个流,需要转换时间基准   */
        if (stream_index == -1 && s->nb_streams == 1) {
            AVRational time_base = s->streams[0]->time_base;
            /*  将时间戳从AV_TIME_BASE转换为流的时间基准    */
    ...
    /*  如果支持read_timestamp接口  */
    if (ffifmt(s->iformat)->read_timestamp) {
        // try to seek via read_timestamp()
        // 尝试使用read_timestamp进行seek
    }

    // Fall back on old API if new is not implemented but old is.
    // Note the old API has somewhat different semantics.
    /*  如果新接口不支持但支持旧接口,回退使用旧的API    */
    if (ffifmt(s->iformat)->read_seek || 1) {
        /*  根据目标时间戳到min_ts和max_ts的距离决定seek方向    */
        int dir = (ts - (uint64_t)min_ts > (uint64_t)max_ts - ts ? AVSEEK_FLAG_BACKWARD : 0);
        /*  尝试第一次seek  */
        int ret = av_seek_frame(s, stream_index, ts, flags | dir);
        /*  如果第一次seek失败且ts不在边界,尝试两段式seek   */
