...
int64_t avio_seek(AVIOContext *s, int64_t offset, int whence) // 在输入或输出流中改变当前的读写位置
{
    FFIOContext *const ctx = ffiocontext(s);
    int64_t offset1;
    int64_t pos;
...
{
    return avio_seek(s, offset, SEEK_CUR); // 在输入或输出流中改变当前的读写位置
}
...
static int read_packet_wrapper(AVIOContext *s, uint8_t *buf, int size) // FFmpeg 中用于封装读取数据包操作的辅助函数
{
    int ret;
...
/* Input stream */
/*  这个函数是 FFmpeg I/O 系统的核心部分，负责管理输入数据的缓冲，确保数据的高效读取和处理。
    它处理了各种边界情况和错误情况，保证了数据读取的可靠性和效率 
*/
static void fill_buffer(AVIOContext *s) //  AVIOContext 的一个关键内部函数，用于从底层 I/O 读取数据以填充 AVIOContext 的缓冲区
{
    // 将 AVIOContext 转换为 FFIOContext
    FFIOContext *const ctx = (FFIOContext *)s; 
    /* 确定最大缓冲区大小，使用 max_packet_size 或默认的 IO_BUFFER_SIZE */
    int max_buffer_size = s->max_packet_size ?
                          s->max_packet_size : IO_BUFFER_SIZE;

    /*决定数据写入的目标位置：如果当前位置加上最大缓冲区大小不超过总大小，则追加；否则从头开始 */
    ...
    */
    ...
    if (s->update_checksum && dst == s->buffer) {
        ...
        s->checksum_ptr = s->buffer; // 重置校验和指针
    }
    ...
    */
    if (s->read_packet && ctx->orig_buffer_size &&
        ...
        len = ctx->orig_buffer_size; // 使用原始缓冲区大小
    }
    ...
    if (len == AVERROR_EOF) {
        /* do not modify buffer if EOF reached so that a seek back can
           ...
           如果遇到 EOF，设置标志但不修改缓冲区，以支持向后搜索
        */
        s->eof_reached = 1;
    } else if (len < 0) {
        /*  如果发生错误，设置 EOF 标志并存储错误代码   */
        s->eof_reached = 1;
        s->error= len;
    } else {
        /* 成功读取数据，更新相关指针和计数器 */
...
int avio_read(AVIOContext *s, unsigned char *buf, int size) // FFmpeg 中用于从 AVIOContext 读取数据的函数
{
    int len, size1;
    ...
    while (size > 0) {
        ...
        if (len == 0 || s->write_flag) {
            if((s->direct || size > s->buffer_size) && !s->update_checksum && s->read_packet) {
                ...
                len = read_packet_wrapper(s, buf, size); // FFmpeg 中用于封装读取数据包操作的辅助函数
                if (len == AVERROR_EOF) {
                    /* do not modify buffer if EOF reached so that a seek back can
                    be done without rereading data */
                    s->eof_reached = 1;
                ...
                fill_buffer(s); //  AVIOContext 的一个关键内部函数，用于从底层 I/O 读取数据以填充 AVIOContext 的缓冲区
...
static int set_buf_size(AVIOContext *s, int buf_size)
{
    ...
    av_free(s->buffer); // 释放内存
    s->buffer = buffer;
    ffiocontext(s)->orig_buffer_size =
    s->buffer_size = buf_size; // 对 FFIOContext 设置 buffer 大小
...
int ffio_rewind_with_probe_data(AVIOContext *s, unsigned char **bufp, int buf_size) // FFmpeg 中用于重新定位 I/O 上下文并整合探测数据的函数
{
