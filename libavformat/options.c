...
{
    FormatContextInternal *fci; // 内部格式上下文
...
#if FF_API_LAVF_SHORTEST    //  API 兼容性处理
 ...
 * 创建并初始化一个新的AVStream
 * @param s 格式上下文
 * @param c 编解码器（可以为NULL）
 * @return 成功返回新创建的流，失败返回NULL
 */
AVStream *avformat_new_stream(AVFormatContext *s, const AVCodec *c)
{
    FFFormatContext *const si = ffformatcontext(s);
    FFStream *sti;                                      // FFStream包含内部实现细节
    AVStream *st;                                       // 公开的流接口
    AVStream **streams;                                 // 用于重分配流数组
    ...
    // 2. 重新分配流数组空间，为新流腾出位置
    streams = av_realloc_array(s->streams, s->nb_streams + 1, sizeof(*streams)); // 添加到流数组并增加计数
    if (!streams)
        return NULL;
    s->streams = streams;

    // 3. 分配新的FFStream结构
    sti = av_mallocz(sizeof(*sti)); // 使用av_mallocz确保内存初始化为0
    if (!sti)
        return NULL;
    st = &sti->pub;                 // 获取公开的AVStream接口
...
// 初始化时间戳相关字段
         ...
         * timestamps corrected before they are returned to the user */
         /* 设置当前DTS为基准值
         * 这样即使没有时间戳的格式也能获得一些时间戳
         * 对于有未知时间戳的格式，其前几个包会被缓存
         * 并在返回给用户前修正时间戳 */
    ...
    // 6. 初始化流的其他属性
    st->index      = s->nb_streams;                 // 设置流索引
    st->start_time = AV_NOPTS_VALUE;                // 初始化开始时间
    st->duration   = AV_NOPTS_VALUE;                // 初始化持续时间
    sti->first_dts     = AV_NOPTS_VALUE;            // 初始化第一个DTS
    sti->probe_packets = s->max_probe_packets;      // 设置探测包数量
    // 设置PTS回环处理参数
