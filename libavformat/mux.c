...
/**
 * 分配并初始化输出格式上下文
 * @param avctx 二级指针，用于返回分配的AVFormatContext
 * @param oformat 指定的输出格式，可以为NULL
 * @param format 格式名称，可以为NULL
 * @param filename 输出文件名，可以为NULL
 * @return 成功返回0，失败返回负的错误码
 */
int avformat_alloc_output_context2(AVFormatContext **avctx, const AVOutputFormat *oformat,
                                   const char *format, const char *filename)
{
    // 1. 分配基础的AVFormatContext结构体
    ...
    *avctx = NULL;
    ...
    // 2. 确定输出格式
    if (!oformat) { // 如果没有直接指定输出格式
        if (format) {
            // 2.1 如果提供了格式名称，根据名称猜测格式
